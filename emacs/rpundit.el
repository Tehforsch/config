...
(defun rpundit/get-pundit-output-from-lines (lines)
  (let
    (
      (filtered-lines
        (cl-remove-if-not #'rpundit/is-output-line lines)))
    (mapcar
      #'(lambda (line) (string-trim line "OUTPUT"))
      filtered-lines)))
...
(defun rpundit/after-term-handle-exit-show-link-append
  (process-name msg)
  (rpundit/cleanup-and-insert-link-text 'rpundit/append-text)
  (advice-remove
    'term-handle-exit
    #'rpundit/after-term-handle-exit-show-link-append))

(defun rpundit/after-term-handle-exit-show-link-insert
  (process-name msg)
  (rpundit/cleanup-and-insert-link-text 'insert)
  (advice-remove
    'term-handle-exit
    #'rpundit/after-term-handle-exit-show-link-insert))
...
(defun rpundit/start (directory command custom-after-term-handle)
  ...
  (let*
    (
      (buf (get-buffer-create "*rpundit*"))
      (min-height (min rpundit/window-height (/ (window-height) 2)))
      (window-height
        (if rpundit/position-bottom
          (- min-height)
          min-height))
      (window-system-args
        (when window-system
    ...
    (setq mode-line-format
      (format "   pundit %s %s" command directory))))
...
(defun rpundit-graph-find ()
  ...
  (rpundit/start
    rpundit/directory
    (s-concat "graph " (buffer-file-name))
    'rpundit/after-term-handle-exit-open-file))
...
(defun rpundit-insert-link ()
  ...
  (rpundit/start
    rpundit/directory
    (s-concat "link " (buffer-file-name))
    'rpundit/after-term-handle-exit-show-link-insert))
...
(defun rpundit-find-backlinks ()
  ...
  (rpundit/start
    rpundit/directory
    (s-concat "backlinks " (buffer-file-name))
    'rpundit/after-term-handle-exit-open-file))
...
(defun rpundit-find-paper ()
  ...
  (rpundit/start
    rpundit/directory
    (s-concat "paper " rpundit/bibtex-file " find")
    'rpundit/after-term-handle-exit-open-file))
...
(defun rpundit/journal-command (command-name journal-name)
  (rpundit/start
    rpundit/directory
    (s-concat "journal " journal-name " " command-name)
    'rpundit/after-term-handle-exit-open-file))
...
(defun rpundit-journal-previous (journal-name)
  ...
  (rpundit/journal-command
    (s-concat "previous " (buffer-file-name))
    journal-name))
...
(defun rpundit-journal-next (journal-name)
  ...
  (rpundit/journal-command
    (s-concat "next " (buffer-file-name))
    journal-name))
...
(defun rpundit-journal-day-before (journal-name)
  ...
  (rpundit/journal-command
    (s-concat "day-before " (buffer-file-name))
    journal-name))
...
(defun rpundit-journal-day-after (journal-name)
  ...
  (rpundit/journal-command
    (s-concat "day-after " (buffer-file-name))
    journal-name))
